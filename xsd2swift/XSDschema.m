...
#import "xsd2swift/XSDschema+XPaths.h"
...
#import "xsd2swift/XSDcomplexType.h"
#import "xsd2swift/XSDelement.h"
#import "xsd2swift/XSSimpleType.h"

#include "xsd2swift/resources/template_swift_xml.h"

@class DDSimpleFormatter;
...
- (void)setTargetNamespacePrefixOverride:(NSString*)prefix {
    ...
    if(!self.targetNamespacePrefix) {
      ...
      if (self.targetNamespacePrefix.length >= 1
          && [[NSCharacterSet uppercaseLetterCharacterSet] characterIsMember:
              [self.targetNamespacePrefix characterAtIndex:0]]) {
 ...
 * Parameters:  (NSData*) The specified template data as to where we are basing our simple types on and code to generate
...
- (BOOL) loadTemplate:(NSData*)templateData error:(NSError**)resultError {
    NSParameterAssert(templateData);

    NSError *error = nil;

    /* Load the template xml document */
    NSXMLDocument* xmlDoc = [[NSXMLDocument alloc] initWithData:templateData
    ...
    for (XSDschema *s in self.includedSchemas) {
        BOOL br = [s loadTemplate:[[self class] swiftTemplate] error:&error];
        if(!br) {
            return NO;
        }
    }
...
+ (NSData*)swiftTemplate {
  static NSData *swiftTemplate;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    swiftTemplate = [NSData dataWithBytes:xsd2swift_resources_template_swift_xml
        length:xsd2swift_resources_template_swift_xml_len];
  });
...
#include "xsd2swift/resources/name_changes_xml.h"
...
//        NSURL *url = [[NSBundle bundleForClass:[self class]] URLForResource:@"nameChanges" withExtension:@"xml"];
        NSData* data = [NSData dataWithBytes:xsd2swift_resources_name_changes_xml
            length:xsd2swift_resources_name_changes_xml_len];
