...
#import "xsd2swift/XSDexplicitGroup.h"
...
@implementation XSDcomplexType {
  ...
  MGTemplateEngine* engine2;  // we need this because else we would recurse and recursion within a
                              // single engine is bad
}
...
- (id)initWithNode:(NSXMLElement*)node schema:(XSDschema*)schema {
  ...
  if (self) {
    ...
    /* For each element that is an attribute, create it as an attribute node and assign all to the
...
- (NSArray*)enumTypesInUse {
  id rtn = [[self simpleTypesInUse]
      filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"hasEnumeration=YES"]];
  return rtn;
}
...
- (NSString*)readSimpleContent {
  ...
  if (baseType != nil) {
    ...
    if (stype != nil && [stype isKindOfClass:[XSSimpleType class]]) {
      ...
      if (substr) [str appendString:substr];
...
- (NSString*)combinedReadPrefixCode {
  NSMutableOrderedSet* lines =
      [NSMutableOrderedSet orderedSetWithCapacity:self.simpleTypesInUse.count];
