...
+ (NSString*)rootNodeNameFromURL:(NSURL*)url {
  ...
  int status;
  for (; (status = xmlTextReaderRead(reader)) &&
         xmlTextReaderNodeType(reader) == XML_READER_TYPE_COMMENT;)
    ;

  if (status) {
    obj = [NSString stringWithCString:(char*)xmlTextReaderConstLocalName(reader)
 ...
 * Details: This public method will iterate the given element looking for children with the given
 * name and will return the array of children if they exist. Otherwise it will return an empty array
 */
