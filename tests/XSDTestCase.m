...
+ (void)helpSetUp {
  ...
  assert(tmpFolderUrl);
  BOOL bCreated = [[NSFileManager defaultManager] createDirectoryAtURL:tmpFolderUrl
...
- (void)helpSetUp {
  ...
  NSURL *schemaUrl =
      [[NSBundle bundleForClass:self.class] URLForResource:self.schemaName withExtension:@"xsd"];
  NSURL *xmlFileUrl =
      [[NSBundle bundleForClass:self.class] URLForResource:self.xmlFileName withExtension:@"xml"];
  assert(schemaUrl);
  assert(xmlFileUrl);
...
- (NSString *)compiledParserPath {
  if (_tmpFolderUrl) {
    id fileName = [NSString stringWithFormat:@"%@%@-parser.dylib", self.schemaName,
                                             self.prefixOverride ? self.prefixOverride : @""];
...
- (void)helpTestCorrectnessParsingSchema {
  __block XSDschema *schema = [[XSDschema alloc] initWithUrl:self.schemaUrl
                                       targetNamespacePrefix:self.prefixOverride
                                                       error:nil];
  XCTAssert(schema);
...
- (void)helpTestCorrectnessGeneratingParser {
  XSDschema *schema = [[XSDschema alloc] initWithUrl:self.schemaUrl
                               targetNamespacePrefix:self.prefixOverride
                                               error:nil];
  XCTAssert(schema);
  ...
  id srcFolder = [_tmpFolderUrl URLByAppendingPathComponent:folderName isDirectory:YES].path;
  folderName = [NSString stringWithFormat:@"%@%@-Sources", self.schemaName,
                                          self.prefixOverride ? self.prefixOverride : @""];
  id destFolder = [_tmpFolderUrl URLByAppendingPathComponent:folderName isDirectory:YES].path;
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    id root = [wlfg_class performSelector:NSSelectorFromString(self.parseMethodName)
                               withObject:self.xmlFileUrl];
...
- (void)helpTestPerformanceParsingSchema {
  ...
  [self measureBlock:^{
    schema = [[XSDschema alloc] initWithUrl:self.schemaUrl
                      targetNamespacePrefix:self.prefixOverride
                                      error:nil];
  }];
  XCTAssert(schema);
...
- (void)helpTestPerformanceParsingXML {
  ...
  BOOL bWritten;
  bWritten =
      [schema generateInto:_tmpFolderUrl products:XSDschemaGeneratorOptionSourceCode error:&error];
  // fix folder name
  id folderName = @"Sources";
  id srcFolder = [_tmpFolderUrl URLByAppendingPathComponent:folderName isDirectory:YES].path;
  folderName = [NSString stringWithFormat:@"%@%@-Sources", self.schemaName,
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    root = [wlfg_class performSelector:NSSelectorFromString(self.parseMethodName)
                            withObject:self.xmlFileUrl];
#pragma clang diagnostic pop
  }];
  XCTAssert(root);
